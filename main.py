...
import os
import file_gen
from mumax_simulation import Mumax_simulation
import mumax3_read
...
if __name__ == '__main__':
    ...
    filename = "z_added.mx3"
    pathBasic = os.getcwd()
    pathOut = os.getcwd() + "/mumax3Gen_0_45/"
    ...
    angle_start = 0.01
    angle_scale = 1
    num_angle = int((45 - angle_start) / angle_scale)
    ...
    for i in range(len(fileName)):
        End_mz = Mumax_simulation(pathBasic=path_simulation,
                                  filename=fileName[i],
                                  pathOut=pathStorage)
        End_mz_list.append(End_mz)
    
    with open('45_89.5_simualtion.csv', 'w', newline='') as student_file:
